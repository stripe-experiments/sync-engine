...
import { z } from 'zod'
import { createConnectorSchemas } from './createSchemas.js'
...
describe('createConnectorSchemas', () => {
  it('preserves connector payload fields for anyOf config schemas', () => {
    const { PipelineConfig } = createConnectorSchemas(resolverWithUnionConfig())

    const parsed = PipelineConfig.parse({
      source: {
        type: 'postgres',
        postgres: {
