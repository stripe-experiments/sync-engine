...
describe('POST /pipelines/:id/simulate_webhook_sync', () => {
  it('fetches events from Stripe and pipes them through the source read() as $stdin', async () => {
    const fakeEvents = [
      { id: 'evt_1', type: 'customer.created', data: { object: { id: 'cus_1' } } },
      { id: 'evt_2', type: 'customer.updated', data: { object: { id: 'cus_2' } } },
    ]

    const receivedInputs: unknown[] = []

    // A stripe source that captures what it receives via $stdin
    const captureSource: Source<Record<string, unknown>> = {
      async *spec() {
        yield {
          type: 'spec',
          spec: {
            config: z.toJSONSchema(
              z.object({ api_key: z.string(), api_version: z.string() })
            ),
          },
        }
      },
      async *check() {
        yield { type: 'connection_status', connection_status: { status: 'succeeded' as const } }
      },
      async *discover() {
        yield { type: 'catalog', catalog: { streams: [] } }
      },
      async *read(_init: unknown, $stdin?: AsyncIterable<unknown>) {
        if (!$stdin) return
        for await (const item of $stdin) {
          receivedInputs.push(item)
        }
      },
    }

    const captureResolver = await createConnectorResolver({
      sources: { stripe: captureSource },
      destinations: { test: destinationTest },
    })

    const pipelineStore = memoryPipelineStore()
    await pipelineStore.set('pipe_webhook', {
      id: 'pipe_webhook',
      source: {
        type: 'stripe',
        stripe: { api_key: 'sk_test_123', api_version: '2025-03-31.basil' },
    ...
    const webhookApp = createApp({ resolver: captureResolver, pipelineStore })

    // Mock makeClient to return our fake events
    vi.mock('@stripe/sync-source-stripe/client', () => ({
      makeClient: () => ({
        listEvents: async () => ({ data: fakeEvents, has_more: false }),
      }),
    }))

    const res = await webhookApp.request('/pipelines/pipe_webhook/simulate_webhook_sync', {
      method: 'POST',
    })
    ...
    expect(receivedInputs).toEqual(fakeEvents)
  })
})
