...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (isHarnessRunning()) {
      ...
      try {
        execPhase('bun run scripts/explorer-harness.ts stop', 'Pre-cleanup')
      } catch {
    ...
    execPhase('bun run scripts/explorer-harness.ts start', 'Start Harness DB')
    ...
    try {
      execPhase(
        `bun run scripts/explorer-migrate.ts --api-version=${config.apiVersion}`,
        'Migrate schema',
        {
    ...
    try {
      execPhase(
        `bun run scripts/explorer-seed.ts --api-version=${config.apiVersion} --seed=${config.seed}`,
        'Seed data',
        {
          STRIPE_API_VERSION: config.apiVersion,
    ...
    execPhase('bun run scripts/explorer-export.ts', 'Export Artifact')
    ...
    execPhase('bun run scripts/explorer-harness.ts stop', 'Stop Harness DB')
    ...
    console.log('   - packages/visualizer/public/explorer-data/bootstrap.sql')
    console.log('   - packages/visualizer/public/explorer-data/manifest.json')
    console.log('')
    ...
    if (shouldCleanup && isHarnessRunning()) {
      ...
      try {
        execSync('bun run scripts/explorer-harness.ts stop', { stdio: 'inherit' })
        console.error('\n✅ Cleanup successful\n')
      } catch (cleanupError) {
        console.error('\n⚠️  Cleanup failed. You may need to manually stop the container:\n')
        console.error('   bun run scripts/explorer-harness.ts stop\n')
      }
    }
