 ...
 *
 * NOTE: This script requires the old stripe-sync-engine monolith's migration
 * system (packages/sync-engine), which is not part of this v2 monorepo.
 * It is preserved here for reference. To regenerate bootstrap.sql/manifest.json,
 * run this script from the original monolith repo (PR #142 branch).
 *
 * Usage:
 *   bun run scripts/explorer-migrate.ts [--api-version=2020-08-27]
 *   STRIPE_API_VERSION=2023-10-16 bun run scripts/explorer-migrate.ts
 */
...
// Stub for the old monolith's runMigrations — not available in v2.
async function runMigrations(_config: {
  databaseUrl: string
  logger?: unknown
  ...
  throw new Error(
    'runMigrations is not available in v2. Run this script from the original monolith repo (PR #142 branch).'
  )
}
...
function printUsage(): void {
  console.log('Usage: bun run scripts/explorer-migrate.ts --api-version=2020-08-27')
  console.log('')
  console.log('Flags:')
  console.log(
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(METADATA_FILE)) {
    ...
    console.error('\n💡 Start the harness first: bun run scripts/explorer-harness.ts start')
    process.exit(1)
  }
