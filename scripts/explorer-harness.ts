 ...
 *
 * Usage:
 *   bun run scripts/explorer-harness.ts start   # Create and start container
 *   bun run scripts/explorer-harness.ts stop    # Stop and cleanup container
 */
...
async function startContainer(): Promise<void> {
  ...
  console.log(`\n🛑 To stop: bun run scripts/explorer-harness.ts stop`)
}
...
async function statusCheck(): Promise<void> {
  ...
  if (!fs.existsSync(METADATA_FILE)) {
    ...
    console.log(`\n💡 Start with: bun run scripts/explorer-harness.ts start`)
    process.exit(0)
  }
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    default:
      console.error('Usage:')
      console.error('  bun run scripts/explorer-harness.ts start   # Start container')
      console.error('  bun run scripts/explorer-harness.ts stop    # Stop and cleanup')
      console.error('  bun run scripts/explorer-harness.ts status  # Check status')
      process.exit(1)
  }
}
