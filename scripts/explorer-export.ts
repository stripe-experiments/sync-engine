...
async function generateInsertStatements(
  ...
  if (insertableColumns.length === 0) {
    return result.rows.map(() => `INSERT INTO ${qualifyTable(tableName)} DEFAULT VALUES;`).join('\n');
  }
  ...
  for (const row of result.rows) {
    ...
    inserts.push(`INSERT INTO ${qualifyTable(tableName)} (${columnList}) VALUES (${values.join(', ')});`);
  }
...
async function exportDatabase(client: Client): Promise<void> {
  ...
  if (tableExports.some((tableExport) => tableExport.tableInfo.constraints.length > 0)) {
    ...
    sqlParts.push('-- Foreign keys and exclusion constraints are omitted for PGlite hydration compatibility.');
    sqlParts.push('');
  ...
  if (tableExports.some((tableExport) => tableExport.tableInfo.triggers.length > 0)) {
    sqlParts.push('-- Trigger functions and triggers are intentionally omitted from the browser artifact.');
