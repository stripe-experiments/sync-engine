...
/**
 * Snapshot the list of streams produced by discover() with and without the
 * source's syncability filter. Catches unintentional changes to what we sync.
 */
describe('catalogFromOpenApi stream list', () => {
  const resolved = resolveOpenApiSpec({ apiVersion: BUNDLED_API_VERSION }, fetch)
  const parser = new SpecParser()

  it('default: only syncable tables', async () => {
    const { spec, apiVersion } = await resolved
    const allowedTables = discoverSyncableTables(spec, OPENAPI_RESOURCE_TABLE_ALIASES)
