  ...
  useEffect(() => {
    ...
    async function initializeDatabase() {
      try {
        ...
        if (!dataArtifact) {
          throw new Error('No data artifact found. Expected /explorer-data/bootstrap.sql or bootstrap.json');
        }
...
async function hydrateSqlBootstrap(db: PGliteInstance, sqlPath: string): Promise<void> {
  ...
  // Execute SQL dump
  // Split on semicolons but be careful with quoted strings and comments
  ...
  console.log(`[PGlite] Executing ${statements.length} SQL statements...`);
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      ...
      console.error(`[PGlite] Error executing statement ${i + 1}:`, statement.substring(0, 100));
      throw err;
    }
  }
...
async function hydrateJsonBootstrap(
  ...
  console.log(`[PGlite] JSON bootstrap size: ${(JSON.stringify(jsonData).length / 1024).toFixed(2)} KB`);
  ...
  for (const tableName of tables) {
    ...
    for (const row of rows) {
      try {
        await db.query(
          `INSERT INTO stripe.${tableName} (_raw_data, _account_id) VALUES ($1, $2)`,
          [JSON.stringify(row._raw_data), row._account_id]
        );
      } catch (err) {
